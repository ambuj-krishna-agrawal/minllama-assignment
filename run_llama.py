...
class LlamaDataset(Dataset):
    ...
    def pad_data(self, data):
        ...
        encoding_padded = [sentence + [self.tokenizer.pad_id] * (max_length_in_batch - len(sentence)) for sentence in
                           encoding]
...
def train(args, fine_tuning_config):
    ...
    model = LlamaEmbeddingClassifier(config, fine_tuning_config)
...
def test(args, fine_tuning_config):
    assert args.dev_out.endswith(
        "dev-finetuning-output.txt"), 'For saving finetuning results, please set the dev_out argument as "<dataset>-dev-finetuning-output.txt"'
    assert args.test_out.endswith(
        "test-finetuning-output.txt"), 'For saving finetuning results, please set the test_out argument as "<dataset>-test-finetuning-output.txt"'
    with torch.no_grad():
        device = torch.device('cuda') if args.use_gpu else torch.device('cpu')
...
def get_args():
    ...
    parser.add_argument("--option", type=str,
                        ...
                        choices=('generate', 'prompt', 'finetune', 'finetune_efficient'), default="generate")
    ...
    parser.add_argument("--lr", type=float, help="learning rate, default lr for 'pretrain': 1e-3, 'finetune': 1e-5",
                        default=2e-5)
    parser.add_argument("--lora_enabled", type=bool, default=False)
    parser.add_argument("--adapter_enabled", type=bool, default=False)
    parser.add_argument("--lora_rank", type=int, default=32)
    parser.add_argument("--adapter_dimension", type=int, default=8)
...
if __name__ == "__main__":
    ...
    if args.option == "generate":
    ...
    elif args.option == "finetune" or args.option == "finetune_efficient":
        fine_tuning_config = {
        ...
        train(args, fine_tuning_config)

        # Step 4
        # Evaluate your model on the dev and test sets
        test(args, fine_tuning_config)
    else:
